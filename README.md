...
Prior to launching the application, ensure that the OpenRouter API endpoint is strictly defined in the `config/` files (or via environment variables). This API connection is required for the Large Language Model Neural Adjudication assistance to function correctly. The local SQLite database maps and indexes will be initialized automatically.
